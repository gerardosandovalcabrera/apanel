...
Integration between Cost Tracking and Plans & Limits
...
class BillingIntegration:
    ...
    def create_budget(
        ...
        Args:
            ...
            alert_thresholds: Thresholds for alerts (e.g., [0.5, 0.8, 1.0])
    ...
    def get_cost_optimization_suggestions(
        ...
        Args:
            organization_id: Organization identifier
            model_usage: Dictionary mapping model_id to token count
        ...
        if total_tokens > 100000:
            suggestions.append({
                ...
                'priority': 'medium',
                'message': "Consider batch processing to reduce API calls",
...
class DemoBillingIntegration:
    """Demo implementation of billing integration"""
    
    def __init__(self):
        self.billing = BillingIntegration()
        self.billing.create_budget(
    ...
    def record_api_call(
        ...
        Args:
            organization_id: Organization identifier
            provider_model_id: Model identifier (e.g., 'openai/gpt-4o')
    ...
    def get_billing_summary(self, organization_id: str) -> Dict:
        ...
        budget_status = None
        budget = next(
            (b for b in self.billing.budgets.values() 
             if b.organization_id == organization_id),
            None
        )
        
        if budget:
            alerts = self.billing.check_budget_alerts(organization_id, total_cost)
            budget_status = {
        ...
        suggestions = self.billing.get_cost_optimization_suggestions(organization_id, model_usage)
        ...
        return {
            ...
            'alerts': [a.to_dict() for a in self.billing.alerts[-5:]],  # Last 5 alerts
...
if __name__ == "__main__":
    ...
    billing = DemoBillingIntegration()
    ...
    for model_id, stats in summary['model_breakdown'].items():
        print(f"   • {model_id}:")
        print(f"     Cost: ${float(stats['cost']):.6f}")
    ...
    print("\n✅ Billing integration test completed successfully!")
